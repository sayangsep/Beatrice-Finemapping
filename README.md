# [BEATRICE: Bayesian Fine-mapping from Summary Data using Deep Variational Inference](https://academic.oup.com/bioinformatics/advance-article/doi/10.1093/bioinformatics/btae590/7808857)
...
- ```--prior_location```:Location to the file where prior probability is stored. The format should be same as the z-file, but instead of z-scores the second column should contain the prior probabilities (default: constant probability ($1/{NumberofVariants}$)).
 ...
 - ```--neural_network```: A comma separated list that contains the size of each layer of the neural network (default: '10,200,10', First layer has 10 nodes, the second layer has 200 nodes and the third has 10 nodes')
